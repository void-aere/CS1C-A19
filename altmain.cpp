...
int main(int argc, char* argv[]) {
    ...
    // std::copy(dataset, dataset + length, ins);

    // long long mer[length];
    // std::copy(dataset, dataset + length, mer);
    // std::cout << "done." << std::endl;

    // sort!
    std::cout << "sorting... (merge) " << std::flush;
    mergeSort(dataset, 1, length);
    ...
    // std::cout << "sorting... (insertion) " << std::flush;
    // insertionSort(ins, length);
    // std::cout << "done (" << verify(ins, length) << ").\n";
}
